...
def generate_role(p, role, env):
  ...
  for instance in server_group['instances']:
    ...
    instance_kind = name_instance_kind(env, instance_size, region_index, subregion_index)

    instance_list = []

    for i in range(instance['number']):
      instance_name = "%s-%s-%d-%d-%s" % (server_names[instance_count], env, region_index, subregion_index, p['domain'])
      ...
      instance_props = {
        ...
        'environment': env # TODO should we store and retrieve it from p['env']?
      }
...
def main():
  ...
  if args.profiles:
    ...
    r = generate_role(p, args.role, args.env)
    print dump(r, default_flow_style=False)
  else:
